...
unsigned char header_storage[54];
unsigned char img_storage[3][512][512];
...
void processing(void) {
  ...
  for(h=0;h<height-16;h++) {
    for(w=0;w<width-16;w++) {
      ...
      for(hy=0; hy<16; hy++) {
        for(wx=0; wx<16; wx++) {
          sum += abs(img_storage[0][x+wx][y+hy] - imgin[0][w+wx][h+hy]);
        }
      }
      if(h == 0 && w == 0)
        min = sum;
  ...
  for(h=0; h<16; h++) {
    for(w=0; w<16; w++) {
      if(h == 0 || w == 0 || h == 15 || w == 15) {
        img_storage[1][w+x][h+y] = imgout[1][w+cw][h+ch] = 128;
        img_storage[2][w+x][h+y] = imgout[2][w+cw][h+ch] = 255;
      }
    }
  }
}
...
void save_img(void) {
  memcpy(header_storage, header, sizeof(header));
  memcpy(img_storage, imgin, sizeof(imgin));
}

void load_img(void) {
  memcpy(header, header_storage, sizeof(header));
  memcpy(imgout, img_storage, sizeof(imgout));
  width = cal_value(18, 4);
  height = cal_value(22, 4);
}
...
void swap_img(void) {
  ...
  memcpy(header_tmp, header, sizeof(header));
  memcpy(header, header_storage, sizeof(header));
  memcpy(header_storage, header_tmp, sizeof(header));
  memcpy(img_tmp, imgout, sizeof(imgout));
  memcpy(imgout, img_storage, sizeof(imgout));
  memcpy(img_storage, img_tmp, sizeof(imgout));
  width = cal_value(18, 4);
  height = cal_value(22, 4);
